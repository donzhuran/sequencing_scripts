...
######################################################## WORKING DIRECTORY
setwd("/Users/jurankralj/Desktop/2022-Summer-School-of-Bioinformatics/2022-08-31-Transcriptomics-Filip")
...
sample_table_dds <-
  sample_table %>%
  mutate(genotype = factor(genotype, levels = c("Ago2_WT", "Ago2_KO", "Ago2_CI", #factor is a type of data that is ordered - ordering genotypes by type
                                                "Dicer_WT", "Dicer_KO"))) %>% 
  as.data.frame(.) %>%
...
colnames(se) <- str_remove_all(colnames(se), "\\.genome\\.Aligned\\.sortedByCoord\\.out\\.bam$") #uzima colnames(se) i briše ovaj dio u navodnicima
